...
void updtCont(chainedTable *countries)
{
    ...
    printf("The current time difference(from the prime meridian) is: %u. \nDo you wish to update it 1 = yes, 0 = no: ", info->timeDiff);
    if (userNum())
    {
        printf("enter the new time difference: ");
        info->timeDiff = userNum();
    }
}
...
{ //country population
    ...
    if (computerTests)
    {                   //compter tests: rand name
        srand(time(0)); //for the rand name
        printf("Enter the number of countries to hash: ");
        unsigned int tests = userNum();
        contName *name;
    ...
    printf("Enter an estimate on the number of countries to be hashed: ");
    numOfElements = (double)(userNum() + 1) * HASH_RATIO;
    chainedTable contries(numOfElements, hash, sameId, freeItem);
    unsigned int userAct;
    do
    {
        printf("\nYou can:\n1: add a country\n2: remove a contry\n3: update information about a county\n4: Get \
