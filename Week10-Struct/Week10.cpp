...
int q1(int *a, int n, int len)
{
	int sum = 0;
	int i = 0;
	int numOfIdamsInArray = n * len;
	while (i < numOfIdamsInArray)
	{
		sum += *a;
		i++;
		a++;
...
void q2(void)
{
	printf("Enter a phrase that you wnat reverse: ");
	char input[100];
	char *p = &input[0];
	char ch = getchar();
	while (ch != '\n')
...
//online question 1 from bacisis in struct
//the diffence between two things
//in A. the Point is a type, but in B. the P is variable of type {double x, double y}

//online question 2 from bacisis in struct
//debuging code
//POINT is a variable not a type so 'POINT a;' does not work becase that is just two undefiend variables next to eachother  

//online question 3 from bacisis in struct
/*struct POINT{double x; double y;} test;
const POINT a;
test.x = .25; test.y = .75;*/

//online question 4 from bacisis in struct
...
//online question 5 from bacisis in struct
...
//online question 6 from bacisis in struct
...
//online question 7 from bacisis in struct
...
//online question 8 from bacisis in struct
...
//online question 9 from bacisis in struct
...
//online question 10 from bacisis in struct
