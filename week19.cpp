...
void myQ_Sort(void* toSort, size_t sizeOfElement, int numOfElements, int(*compareFunc)(const void* p1, const void* p2))
{
	...
	if (numOfElements <= 2)
		return;
	myQ_Sort(toSort, sizeOfElement, ((char*)low - toSort)/sizeOfElement, compareFunc); //sort the small side
	myQ_Sort((char*)high + sizeOfElement, sizeOfElement, numOfElements - (((char*)low - toSort) / sizeOfElement) - 1, compareFunc); //sort the small side
}
