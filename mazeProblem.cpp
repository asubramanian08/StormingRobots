...
#define DEFALUT_DIMENTION 100
tileNum numOfRows = 3;
tileNum numOfCols = 6;
unsigned long long numOfSquares;
preMap* givenGrid;
enum bits {per = 1 << 5, N = 1 << 4, E = 1 << 3, S = 1 << 2, W = 1 << 1, vis = 1 };
uByte* newGrid;
...
queue* printGrid(queue * curr)
{
	...
	tileNum j;
	for (j = 0; j < numOfCols; j++) { //top line (top most wall)
		putc(' ', stdout);
		if (givenGrid[j] & N)
			putc('_', stdout);
		else
	...
	for (tileNum i = 0; i < numOfRows; i++) { //go through the rest
		if (givenGrid[i * numOfCols] & W)
			...
			putc(' ', stdout);
		for (j = 0; j < numOfCols; j++) { //person + left, right walls
			if (givenGrid[i * numOfCols + j] >> 8) { //something to put there
		...
		putc('\n', stdout);
		for (j = 0; j < numOfCols; j++) { //the top and bottem walls
			putc(' ', stdout);
...
{
	if (argc <= 1) {
		printf("MAIN: User Failer -> grid has not been given\n");
		exit(EXIT_FAILURE);
	}
	//get the rows and cols
	numOfRows = DEFALUT_DIMENTION;
	numOfCols = DEFALUT_DIMENTION;
	tileNum* location;
	...
	if ((fp = fopen(argv[1], "r")) == NULL) {
		printf("MAIN: User failer -> invaled grid location!!!\n");
		exit(EXIT_FAILURE);
	}
	getGrid(fp);
	fclose(fp);
	...
	while (notDone()){
		...
		while (getchar() != '\n');
		for (i = 0; i < 4; i++) { //find a dir I can go
