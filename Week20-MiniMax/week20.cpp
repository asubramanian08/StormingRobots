...
void printGrid (void) //print the current grid nicly
{
...
int mini_max(bool toPlace, char** spot, int prevHigh) //actual program | recusion | 1 - win, 0 - tie, -1 -
{
	int currHigh = -2;
	int curr;
	char* junk = NULL; //something I give to the next func call

	for (int i = 0; i < 9; i++) { //check every position
		if ((grid[i] != 'X') && (grid[i] != 'O')) { //if not filled
			grid[i] = moves[toPlace]; //try out a placement in ith location temorarily
			if (!winCheck(i)) { //if what I just said didn't win anything
				curr = mini_max(!toPlace, &junk, currHigh); //call recusion
				if (-1 * curr > currHigh) { //if this is an improvement from what I orginally have
					*spot = grid + i;
					currHigh = -1 * curr; //the curr is of the opposite type
					if (optimization && (-1 * currHigh < prevHigh)){ //prunning
...
int main(void) //setup + running tests + end display
{
	...
	while(getchar() != '\n'); //till user enters
	if (testType){ //acuracy -> find times to run
		timesRun = 0;
	...
	int currPlace = 0;
	while (timesPlayed < timesRun){ //run game timesRun number of times
		currPlace = 0;
		while (currPlace < 9) {//set grid
		...
		while (currPlace < 9){ //game cycle
			...
			if (currPlace % 2 == usersMove) { //get non-program generated number
				if (!testType) {  //performance -> asking user where to go
					do {
						printGrid();
				...
				else { //accuracy - getting rand location
					locationOfPlacement = grid + rand() % 9;
					while (*locationOfPlacement == 'X' || *locationOfPlacement == 'O') { //while this is not possible
		...
		if (!testType) {//performance -> indicate endgame type
			printGrid();
			if (currPlace == 9)
				printf(win[1]);
		...
		else { //accuracy -> increment endgame type
			if (currPlace == 9)
				numOfTie++;
			else if (currPlace % 2 != usersMove)
				numOfLoss++;
