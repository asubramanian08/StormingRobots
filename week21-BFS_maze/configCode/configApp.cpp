...
int main(int argc, char *argv[])
{
    ...
    while (*readPtr == nextItem)
    {
        ...
        readPtr++;
        if (*(readPtr - 1) == range)
            rowHigh = getNum(&readPtr); //inclusive
        else
        {
            readPtr--;
            rowHigh = rowLow;
        ...
        if (tolower(*readPtr) == 'e')
        {
            ...
            colLow = getNum(&readPtr) + 1; //inclusive
        ...
        if (tolower(*readPtr) == 'e')
        {
            ...
            colHigh = getNum(&readPtr) + 1; //exclusive
        //put info into grid
        temp = colLow;
        while (rowLow <= rowHigh)
        {
    ...
    if (argc >= 3)
    {
        ...
        {
            printf("MAIN: User error -> invalid location for the grid location\n");
            //clean up
            store = 2 * numOfRows + 1;
            for (dimension i = 0; i < store; i++)
                free(grid[i]);
