...
FILE *checkFile(const char *fileName, const char *fileMode)
{
    ...
    if (!fp)
    {
        printf("FILE I/O error has ocurred\n\
...
If you need furthar instructions on how to operate this compression/decompression algorithm run the code with mode i\n");
        exit(EXIT_FAILURE);
    }
    return fp;
}
...
{
    return ((huffman *)p1)->timesRepeated < ((huffman *)p2)->timesRepeated;
}
huffman *buildTree(void)
{
    ...
    while (rightChild = (huffman *)pQue_deque(sizeof(huffman), compareFunc_minHeap))
    {
        ...
        parent->timesRepeated = leftChild->timesRepeated + rightChild->timesRepeated;
        parent->whichChar = 0;
...
inline void enqueFreqTable(frequency *freqTable)
{
    ...
    do
    {
        if (freqTable[counter])
        {
            newNode->lChild = NULL;
            newNode->rChild = NULL;
            newNode->timesRepeated = freqTable[counter];
...
inline void fillBiteArray(unsigned char *compressedData, bits *conversion, FILE *fp, frequency *currChar, char *bitsLeft)
{
    ...
    *bitsLeft = CHAR_BIT;
    ...
    {
        if (*bitsLeft < conversion[ch].numOfBits)
        { //i need to split it in more than one element
...
void compression(const char *inputFile, const char *outputFile)
{
    ...
    enqueFreqTable(freqTable); //enque all valid node into priority que
    //get the tree
    ...
    const frequency MAX_SPACE = huffman_tree->timesRepeated << 1;
...
inline void enque_items(FILE *fp)
{
    ...
    newNode->lChild = NULL;
    newNode->rChild = NULL;
    fread(&newNode->timesRepeated, sizeof(frequency), 1, fp);
    while (newNode->timesRepeated)
    {
...
inline void printToFile(frequency numCompressedBytes, huffman *huffman_tree, FILE *fp, unsigned char *compressedData, char lastBit)
{
    ...
    numCompressedBytes--; //the last bit is only partially filled and needs to be handled differently
    while (currChar < numCompressedBytes)
    { //loop through the majority of bits
...
void decompression(const char *inputFile, const char *outputFile)
{
    ...
    printToFile(numCompressedBytes, huffman_tree, fp, compressedData, lastBit); //every char is immediately spit into file
    fclose(fp);
...
//marco of input instructions
#define HOW_TO_USE "\nInstructions:\n\n\
 - WARNING: THERE ARE NO DASHES IN FRONT OF ANY ARGUMENT\n\
...
For compression it should be a text file and for decompression a binary file made by the compression algorithm\n\
 - If doing compression or decompression: the next argument is the output file.\n\
This file in not mandatory but if you choose -> the output file type of: commpression will be bianary and decompression will be text\n\n\n\
General:\n\n\
 - This compression technique is called huffman coding\n\
 - WARNING: The maximum frequency of a charactor is %llu\n\
 - The default output file of: compression is \"%s\", and decompression is \"%s\"\n\
 - A priority que is used to actively sort frequencies\n\
 - There is no need for the macro \"pQue_test\" to be turn on and the program actively tries to disable it\n\
